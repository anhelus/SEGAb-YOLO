...
from segab_yolo.nn.modules.conv import ChannelAttention, SpatialAttention
    ...
    def __init__(self, channels, groups=4):
        ...
        for ks in kernel_sizes[:groups]:
            ...
            self.dw_convs.append(
                nn.Conv2d(group_ch, group_ch, ks, 1, pad, groups=group_ch, bias=False)
            )

        self.cross_convs = nn.ModuleList()
        for _ in range(groups):
            self.cross_convs.append(
                nn.Conv2d(group_ch, group_ch, 1, 1, 0, bias=False)
            )
