...
from segab_yolo.utils import LOGGER, DataExportMixin, SimpleClass, TryExcept, checks, plt_settings
from segab_yolo.utils.plotting import colors
...
def boundary_iou(
    ...
    """Calculate Boundary IoU between masks.

    Boundary IoU (Cheng et al., CVPR 2021) focuses evaluation on the boundary region of objects
    rather than the whole mask. It computes IoU within a thin dilated band around object contours.

    Args:
        mask1 (torch.Tensor): Ground truth masks of shape (N, HW), flattened binary masks.
        mask2 (torch.Tensor): Predicted masks of shape (M, HW), flattened binary masks.
        dilation_ratio (float): Fraction of sqrt(mask_area) used for boundary dilation.
        eps (float, optional): A small value to avoid division by zero.

    Returns:
        (torch.Tensor): A tensor of shape (N, M) representing Boundary IoU.
    """
    N, HW = mask1.shape
    M = mask2.shape[0]
    ...
    H, W = side, HW // side
    if H * W != HW:
        return mask_iou(mask1, mask2, eps)

    m1 = mask1.view(N, 1, H, W).float()
    m2 = mask2.view(M, 1, H, W).float()
    device = mask1.device

    k = 3
    kernel = torch.ones(1, 1, k, k, device=device, dtype=m1.dtype)
    k_sum = k * k

    def get_boundary(m):
        conv = torch.nn.functional.conv2d(m, kernel, padding=1)
        return (conv > 0).float() - (conv >= k_sum).float()

    b1 = get_boundary(m1)
    b2 = get_boundary(m2)

    d = max(1, int(dilation_ratio * math.sqrt(H * W)))
    dk = 2 * d + 1
    dilate_kernel = torch.ones(1, 1, dk, dk, device=device, dtype=m1.dtype)

    def dilate_boundary(b):
        return torch.clamp(torch.nn.functional.conv2d(b, dilate_kernel, padding=d), 0, 1)

    band1 = dilate_boundary(b1)
    band2 = dilate_boundary(b2)

    B = torch.clamp(band1[:, None] + band2[None, :], 0, 1)
    G = m1[:, None]
    P = m2[None, :]

    inter = (G * P * B).sum(dim=(-1, -2))
    union = (torch.clamp(G + P, 0, 1) * B).sum(dim=(-1, -2))
...
def smooth_bce(eps: float = 0.1) -> tuple[float, float]:
    ...
    References:
        https://github.com/segab_yolo/yolov3/issues/238#issuecomment-598028441
    """
...
class Metric(SimpleClass):
    """Class for computing evaluation metrics for segab_yolo YOLO models.
